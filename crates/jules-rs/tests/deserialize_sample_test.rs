...
fn test_deserialize_activities_json() {
        ...
        for (j, artifact) in activity.artifacts.iter().enumerate() {
            if let Some(bash) = &artifact.bash_output {
                if let Some(cmd) = &bash.command {
                    assert!(
                        !cmd.is_empty(),
                        "Activity {} artifact {} has empty bash command",
                        i,
                        j
                    );
                }
                if let Some(output) = &bash.output {
                    assert!(
                        !output.is_empty(),
                        "Activity {} artifact {} has empty bash output",
                        i,
                        j
                    );
                }
                println!(
                    "  └─ Artifact {}: bash output (exit: {:?})",
                    j, bash.exit_code
                );
            }
            ...
            if let Some(changeset) = &artifact.change_set {
                assert!(
                    !changeset.source.is_empty(),
                    "Activity {} artifact {} has empty source",
                    i,
                    j
                );
                if let Some(patch) = &changeset.git_patch {
                    if let Some(unidiff) = &patch.unidiff_patch {
                        assert!(
                            !unidiff.is_empty(),
                            "Activity {} artifact {} has empty patch",
                            i,
                            j
                        );
                        println!(
                            "  └─ Artifact {}: git patch ({} lines)",
                            j,
                            unidiff.lines().count()
                        );
                    } else {
                    ...
                    if let Some(base_commit) = &patch.base_commit_id {
                        assert!(
                            ...
                            "Activity {} artifact {} has empty commit id",
                            i,
                            j
                        );
                    }
    ...
    println!(
        "\n✅ Successfully deserialized all {} activities",
        activities.len()
    );
}
...
fn test_bash_output_with_missing_exit_code() {
    ...
    let bash: BashOutput =
        serde_json::from_str(json).expect("Failed to deserialize BashOutput without exitCode");

    assert_eq!(bash.command, Some("\ncd /app\necho do setup\n".to_string()));
    assert_eq!(bash.output, Some("do setup".to_string()));
    assert_eq!(
        bash.exit_code, None,
        "Exit code should be None when missing"
    );
}
...
fn test_bash_output_with_exit_code() {
    ...
    let bash: BashOutput =
        serde_json::from_str(json).expect("Failed to deserialize BashOutput with exitCode");
...
fn test_activity_content_extraction() {
    ...
    println!(
        "Activities with extractable content: {}/{}",
        content_count,
        activities.len()
    );
    assert!(
        content_count > 0,
        "Expected some activities to have content"
    );
}
