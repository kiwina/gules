    ...
    match serde_json::from_str::<Vec<Activity>>(&json) {
        Ok(activities) => {
            ...
            for (i, activity) in response.activities.iter().enumerate() {
                ...
                assert!(
                    !activity.originator.is_empty(),
                    "Activity {} has empty originator",
                    i
                );

                // Count activity types
                if activity.agent_messaged.is_some() {
                    stats.agent_messaged += 1;
                }
                if activity.user_messaged.is_some() {
                    stats.user_messaged += 1;
                }
                if activity.plan_generated.is_some() {
                    stats.plan_generated += 1;
                }
                if activity.plan_approved.is_some() {
                    stats.plan_approved += 1;
                }
                if activity.progress_updated.is_some() {
                    stats.progress_updated += 1;
                }
                if activity.session_completed.is_some() {
                    stats.session_completed += 1;
                }
                if activity.session_failed.is_some() {
