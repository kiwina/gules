...
fn test_filter_real_activities_json() {
    ...
    assert!(counts.get("progress_updated").copied().unwrap_or(0) > 0,
               "Expected some progress_updated activities");
    assert!(counts.get("plan_generated").copied().unwrap_or(0) > 0,
               "Expected some plan_generated activities");
    assert!(counts.get("plan_approved").copied().unwrap_or(0) > 0,
               "Expected some plan_approved activity");
    assert!(counts.get("agent_messaged").copied().unwrap_or(0) > 0,
               "Expected some agent_messaged activity");
    assert!(counts.get("user_messaged").copied().unwrap_or(0) > 0,
               "Expected some user_messaged activity");

    // Test filtering by Progress type
    let progress_filter = ActivityTypeFilter::Progress;
    let filtered_progress: Vec<_> = activities.iter()
        .filter(|a| progress_filter.matches(a))
        .collect();
    
    println!("\nProgress activities: {}", filtered_progress.len());
    assert!(filtered_progress.len() > 0, "Expected some progress activities");

    // Test filtering for activities with bash output
    let with_bash: Vec<_> = activities.iter()
        .filter(|a| {
            a.artifacts.iter().any(|artifact| artifact.bash_output.is_some())
        })
        .collect();
    ...
    assert!(with_bash.len() >= 1, "Expected at least 1 activity with bash output");

    // Test filtering for errors (bash_output with exit_code != 0)
    let errors: Vec<_> = activities.iter()
        .filter(|a| {
            a.artifacts.iter().any(|artifact| {
                artifact.bash_output.as_ref()
    ...
    println!("\nActivities with errors (exit_code != 0): {}", errors.len());
    ...
    let progress_with_bash: Vec<_> = activities.iter()
        .filter(|a| progress_filter.matches(a))
        .filter(|a| a.artifacts.iter().any(|artifact| artifact.bash_output.is_some()))
        .collect();
    
    println!("\nProgress activities with bash output: {}", progress_with_bash.len());
    assert!(progress_with_bash.len() >= 1, "Expected at least 1 progress activity with bash output");
}
...
fn test_detect_bash_output_in_progress_updates() {
    ...
    assert!(bash_activity.progress_updated.is_some(), "Should be a progress_updated activity");
    
    // Verify it has bash output in artifacts
    assert!(
        bash_activity.artifacts.iter().any(|a| a.bash_output.is_some()),
        "Should have bash output in artifacts"
    );
    
    // Verify the Progress filter matches it
    let progress_filter = ActivityTypeFilter::Progress;
    assert!(progress_filter.matches(bash_activity), "Progress filter should match");
}
